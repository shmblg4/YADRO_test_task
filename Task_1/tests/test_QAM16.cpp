...
int main() {
    ...
    try {
        modulator.Modulate(bits, symbols, QAM::QAM16);
    ...
    try {
        demodulator.Demodulate(trans_symbols, demodulated_bits, QAM::QAM16);
    ...
    if (demodulated_bits != bits) {
        std::cerr << "Demodulation failed witht norm" << std::endl;
    } else {
